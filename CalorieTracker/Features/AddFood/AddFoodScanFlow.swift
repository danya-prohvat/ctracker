...
/// scan counting and routing of scan outcomes into the navigation path.
    ...
    var scanFlow: some View {
        ScanFlowView(
            ...
            onPrefill: { prefill in
                countScan()
                showScanner = false
                path.append(.scanPrefill(loggable(from: prefill), prefill.barcode))
            },
            onCreateManually: { barcode in
                ...
                showScanner = false
                path.append(barcode.isEmpty ? .newProduct : .manualBarcode(barcode))
            }
        )
    }
